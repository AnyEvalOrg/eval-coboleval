...
The sandbox root filesystem is read-only. Writable `/tmp` (including compiler scratch files, work directories and captured streams) is a disk-backed `emptyDir` with a 512 MiB sizeLimit as an eviction backstop, within the pod’s 1 GiB ephemeral-storage budget. `/dev/shm` remains a memory-backed `emptyDir` with sizeLimit 16 MiB. Production gVisor does not enforce memory-backed emptyDir sizeLimit as a tmpfs size. A second supervisor watchdog scans all regular files under `/tmp` and `/dev/shm` every 100 ms, summing `st_blocks * 512` without following symlinks and tolerating deletion/replacement races. Above 256 MiB it uses the memory watchdog’s immediate group/UID kill path and signs `disk_exceeded=true`, scoring INCORRECT as “disk limit exceeded.” Budget for 256 MiB plus one 100 ms write burst; gVisor gofer write throughput makes the burst small. The production regression checks signed disk failures, independent cleanup and continued pod usability. Docker uses a writable disk volume for `/tmp`, allowing compiled executables to run, and the same watchdog. No external access is needed by the task after image provisioning.
